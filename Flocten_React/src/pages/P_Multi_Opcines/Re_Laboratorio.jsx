      ...
      <div className="px-4 py-2.5 bg-base-200 rounded-lg border flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold">
            {authUser?.controlNumber || "No disponible"}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <User2 className="w-4 h-4" />
          ...
          <span className="font-semibold">
            {authUser?.fullName || "No disponible"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">
            Estado: {authUser ? "Autenticado" : "No autenticado"}
          </span>
        </div>
      </div>
...
const Re_Laboratorio = () => {
  ...
  // Verificación de permisos cuando se selecciona un tipo
  useEffect(() => {
    if (itemType && authChecked && !hasPermission("registro")) {
      alert("No tienes permisos para registrar nuevos items.");
      setItemType("");
    }
  }, [itemType, authChecked, hasPermission]);
  ...
  if (!hasPermission("registro")) {
    return (
      <div className="min-h-screen flex items-center justify-center pt-2">
        <div className="text-center p-10 bg-base-100 shadow-xl rounded-lg">
          <h1 className="text-2xl font-bold text-error">Acceso Denegado</h1>
          <p className="mt-4">
            No tienes permisos para registrar nuevos items.
          </p>
          <p className="mt-4">
            Solicita a tu asesor o a alguien que esté autorizado para que te
        ...
        <div className="p-6 md:p-8 space-y-6 font-bold">
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text">Tipo de Item a Registrar</span>
            </label>
            <select
