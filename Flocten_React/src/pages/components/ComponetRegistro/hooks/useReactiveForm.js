...
export const useReactiveForm = (reactiveId, onClose = () => {}) => {
    ...
    const fetchReactiveData = async () => {
      try {
        const response = await axiosInstance.get(
          `http://localhost:5001/api/reactivos/${reactiveId}`,
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/reactivos/search?term=${encodeURIComponent(searchTerm)}`
      );
      ...
      if (err.response?.status === 401) {
        ...
        // Error de validación del servidor (como término muy corto)
        setError(err.response?.data?.message || "Término de búsqueda inválido");
      } else if (err.response?.status === 500) {
        setError("Error del servidor. Por favor intente nuevamente.");
      } else {
        setError(
          `Error al buscar reactivos: ${
            err.response?.data?.message || err.message
          }`
        );
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosInstance.put(
        `http://localhost:5001/api/reactivos/${reactiveData.codigo}`,
        dataToSend,
        {
          headers: {
